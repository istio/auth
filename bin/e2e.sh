...
bazel run $BAZEL_ARGS //cmd/generate_cert -- \
...
bin/push-docker.sh  -i "$DOCKER_IMAGE" -h $HUB -t $TAG

# Run integration tests
bazel run $BAZEL_ARGS //integration -- $ARGS \
-k $HOME/.kube/config \
--root-cert ${SECURITY_KEY_OUTPUT_DIR}/istio_ca.crt \
--cert-chain ${SECURITY_KEY_OUTPUT_DIR}/istio_ca.crt \
--alsologtostderr
