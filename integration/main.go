...
func runTests(cmd *cobra.Command, args []string) {
	...
	// Check certificate of NodeAgent was updated and valid
	na_service, err :=
...
serviceType v1.ServiceType, pod *v1.Pod) (*v1.Service, error) {
	...
	if serviceType == v1.ServiceTypeLoadBalancer {
		err = waitLoadBalancerForService(uuid, 300 * time.Second)
		if err != nil {
			return nil, err
		}
	}
...
func waitLoadBalancerForService(uuid string, timeToWait time.Duration) error {
