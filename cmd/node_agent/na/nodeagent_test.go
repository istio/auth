...
func TestStartWithArgs(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"SendCSR parsing error": {
			...
			req:         FakePlatformSpecificRequest{nil, "service1", true},
			cAClient:    &FakeCAClient{0, &pb.Response{IsApproved: true, SignedCertChain: []byte(`TESTCERT`)}, nil},
			certUtil:    FakeCertUtil{time.Duration(0), fmt.Errorf("SendCSR parsing error.")},
			expectedErr: "node agent can't get the CSR approved from Istio CA after max number of retries (3)",
			sendTimes:   4,
		},
	}
	...
	for id, c := range testCases {
		...
		fakeFileUtil := mockutil.FakeFileUtil{make(map[string][]byte), make(map[string][]byte)}
		fakeWorkloadIO, _ := workloadio.NewWorkloadIO(
			workloadio.Config{
		...
		if c.cAClient.Counter != c.sendTimes {
			t.Errorf("Test case [%s]: sendCSR is called incorrect times: %d. It should be %d.", id, c.cAClient.Counter, c.sendTimes)
		}
		if c.fileContent != nil && bytes.Compare(fakeFileUtil.WriteContent["cert_file"], c.fileContent) != 0 {
			t.Errorf("Test case [%s]: cert file content incorrect: %s vs. %s.", id, fakeFileUtil.WriteContent["cert_file"], c.fileContent)
		}
	}
}
