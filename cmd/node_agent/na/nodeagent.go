...
func (na *nodeAgentInternal) Start() error {
	...
	for {
		...
		if err == nil && resp != nil && resp.IsApproved {
			waitTime, ttlErr := na.cu.GetWaitTime(
			...
			if ttlErr != nil {
				...
				if writeErr := na.wlio.SetServiceIdentityCert(resp.SignedCertChain); writeErr != nil {
					return writeErr
				}
				if writeErr := na.wlio.SetServiceIdentityPrivateKey(privateKey); writeErr != nil {
					return writeErr
				}
